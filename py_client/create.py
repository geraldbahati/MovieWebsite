...
def download_url_image(movie):
    ...
    save_path = os.path.join(os.getcwd(), "backend/media/movie_posters")
    ...
    file_name = movie["title"] + "_poster.jpg"
    ...
    try:
        # send a request to the URL to download the image
...
genre_url = "http://127.0.0.1:8000/api/genres/"
movie_url = "http://127.0.0.1:8000/api/movies/"
...
#     "director": "gerald",
#     "actors": "greg, greg",
#     # "poster": "path/till/poster.png",
#     "plot": "hello",
   
#     "rating": 1
# }

# response = requests.post(movie_url, headers=header_details, json=data)
# print(response.status_code)

def get_genre(genres):
    genre_list = []
    for genre in genres:
        id = genre_data.index(genre)
        genre_list.append(id+3)

    return genre_list


for movie in movie_data:
    poster = download_url_image(movie)
...
#     "rating": 2.0
# }
        django_file = SimpleUploadedFile(name=poster[0], content=poster[1])

        with open('/Users/geraldbahati/Downloads/2d3c9478286f0830af1ed48007f18191.png', 'rb') as f:
            image_data = base64.b64encode(f.read()).decode('utf-8')

        datum = {
            "title": movie["title"],
            "genre": get_genre(movie["genres"]),
            "year": movie["year"],
            "runtime": movie["runtime"],
            "director": movie["director"],
            "actors": movie["actors"],
            # "poster": ("image.jpg", image_data, "image/jpeg"),
            "plot": movie["plot"],
        ...
        payload = {
            # "data": ("data", json.dumps(datum), "application/json"),
            "poster": ("image.jpg", image_data, "image/jpeg")
        }

        # response = requests.post(movie_url, data=json.dumps(datum), headers=header_details)
        response = requests.post(movie_url, files=datum, headers={'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundary7MA4YWxkTrZu0gW'})
...
#     response = requests.post(genre_url, headers=header_details, json={"name": genre})
#     print(response.status_code)
